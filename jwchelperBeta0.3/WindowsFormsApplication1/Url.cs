	...
	class Url
	{
        ...
        public static Uri GetTrueUrl(string Url)
		{
			...
			try
			{
				...
				writeFile(Login.get().loginPage, Application.StartupPath+"\\login.html");
            }
			catch (Exception ex)
			{
		...
		public static bool PostLogin(string form)
		{
			...
			try
			{
				...
				writeFile(result, @Application.StartupPath+"\\post.html");
		...
		public static Stream getPostStream(string url, string postData)
		{
			...
			try
			{
                ...
                //Login login;
                //login = Login.get();
                //HttpWReq.Referer = login.homeUrl + "xs_main.aspx?xh=" + login.id;
