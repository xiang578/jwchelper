...
namespace jwchelper
{
	class Elective
	{
		...
		{
			login = Login.get();
			url = login.homeUrl + "xf_xsqxxxk.aspx?xh=" + login.id + "&xm=" + login.urlName + "&gnmkdm=N121206";
			int i = 20;
			while (i-- > 0)
			{
			...
			Url.writeFile(page, Application.StartupPath+"\\elective.html");
			...
			Url.writeFile(page, Application.StartupPath+"\\elective.post.html");
